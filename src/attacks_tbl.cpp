...
static AttackTables build() {
  ...
  for (int s = 0; s < 64; ++s) {
    ...
    T.knight_sz[ss] = static_cast<uint8_t>(n);
  }
  ...
  for (int s = 0; s < 64; ++s) {
    ...
    for (int df = -1; df <= 1; ++df) {
      for (int dr = -1; dr <= 1; ++dr) {
        ...
        T.king_to[ss][n++] = r * 8 + f;
      }
    }
    T.king_sz[ss] = static_cast<uint8_t>(n);
  }
  ...
  auto push_ray = [&](int s, int df, int dr, int dir_idx) {
    ...
    while (f >= 0 && f < 8 && r >= 0 && r < 8) {
      T.rays[dir_idx][ss][n++] = r * 8 + f;
      f += df; r += dr;
    }
    T.ray_len[dir_idx][ss] = static_cast<uint8_t>(n);
  };
